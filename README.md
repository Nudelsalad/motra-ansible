...
- las3 user is hardcoded in some places; change the playbooks if needed

Example inventory:

```
[rpiserver]
raspiserver ansible_host=<server_ip> ansible_user=las3 ansible_password=<pwd> ansible_become_password=<pwd>

[rpiclient]
raspiclient ansible_host=<client_ip> ansible_user=las3 ansible_password=<pwd> ansible_become_password=<pwd>
```
...
```
ansible-playbook -i inventory.ini plays/ sched_test_play.yml
```

4. Motra Reset

```
ansible-playbook -i inventory.ini plays/motra_reset_play.yml
```
